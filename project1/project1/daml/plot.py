# plot.py: Collection of plotting methods common to several DAML CPs.
#
...
def confusion_matrix (cm, labels=None, normalise=False, size=6):
    """
    ...
    Arguments:
        ...
        size: Float, size of figure to create.
    ...
    # Create figure
    fig, ax = _plt.subplots(figsize=(size,size))
...
def roc (*clfs, X=None, y=None, weights=None, target_eff=0.5, scale='log'):
    """
    ...
    Arguments:
        clfs: Variable-length list of scikit-learn classifiers, such as
            sklearn.neural_network.MLPClassifier; Keras networks, of type
            keras.models.Model; or tuples of the format (prediction_array, name).
            Classifiers are assumed to have been fitted on the same set of
            features to classify only two target classes either using their
            respective  `fit`-methods, or using the `utilities.fit` method.
            These are specified in the function call as
            e.g.:
            ```
            plot.roc(clf1, clf2, clf3, X=X_test, y=y_test);
            # or
            plot.roc((preds, 'variable'), y=y_test);
            ```
        ...
        weights: numpy.array, of shape (nb_samples, 1), containing the samples
            weights to be used when calculating the ROC curve. If None, all
    ...
    # Check(s)
    assert all(map(lambda clf: isinstance(clf, (tuple, list)), clfs)) or (X is not None), \
        "Please specify a testing dataset, `X`."
    assert y is not None, \
        "Please specify true labels, `y`."
    if weights is None:
        weights = _np.ones_like(y).astype(_np.float32)
    ...
    for clf in clfs:
        ...
        if isinstance(clf, (list, tuple)):
            assert len(clf) == 2
            pred, label = clf
        else:
            pred  = _utilities.get_output(clf, X)
            label = _utilities.get_network_name(clf)
            pass

        # Validation the prediction shape
        if len(pred.shape) > 1:
            assert pred.shape[1] <= 2, "The `roc` method does not support classification of more than two classes."
            if pred.shape[1] == 2:
                pred = pred[:,0]
                pass
            pass
        ...
        fpr, tpr, _ = roc_curve(y, sign * pred, sample_weight=weights)
        ...
        # Perform interpolation to per-mill spacing
        ix  = _np.interp(_np.linspace(0, 1, 1000 + 1), tpr, _np.arange(len(tpr))).astype(int)
        tpr = tpr[ix]
        fpr = fpr[ix]
...
def optimisation_grid (clf, fmt="{:.1f}%", scale=100.):
    ...
    if len(dims) == 1:
        ...
        line = ax.plot(x, yval, 's-', markersize=4)
...
def optimisation_gp (bo, fmt="{:.1f}%", scale=100.):
    ...
    if len(bo.dimensions) == 1:
        ...
        for ix, ax in enumerate(axes):
            ...
            yval, yerr = func(bo.plotX)

            line = ax.plot(bo.plotX, yval)
            if yerr is not None:
                fill = ax.fill_between(bo.plotX.flatten(), yval - yerr, yval + yerr, alpha=0.2)
                pass
            ...
            if   ix == 0:
                ...
                ax.set_ylim(0, _np.max(yval) * 1.2)
    ...
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        for ix, ax in enumerate(axes):
            if ix == 0:
                ...
                pass
            yval = func(bo.plotX).reshape(bo.plotmesh[0].shape)
            ...
            im = ax.contourf(bo.plotmesh[0], bo.plotmesh[1], yval, levels=11, cmap='Blues', vmin=vmin, vmax=vmax)
            ...
            for trial in bo.trials:
                ...
                left   = (x - xmin) / dx < 0.90
                bottom = (y - ymin) / dy < 0.90
                ax.scatter([x], [y], color=colour, s=10)
                ax.text(x + 0.01 * dx * (1 if left   else -1),
                        y + 0.01 * dy * (1 if bottom else -1),
            ...
            cbar = fig.colorbar(im, ax=ax, format='%.3f' if ix == 2 else None)
            if   ix == 0:
...
def image (img, size=4, title=None, cmap='gray', xlabel=None, ylabel=None,
           ...
           symmetric=False, vmin=None, vmax=None, titlesize=16, layout=None, ax=None):
    """
    Display image(s).
    ...
    Arguments:
        ...
        layout: Tuple of integers, specifying layout of image grid.
    ...
    kwargs = dict(size=size, cmap=cmap, xlabel=xlabel, ylabel=ylabel,
                  ...
                  symmetric=symmetric, vmin=vmin, vmax=vmax, titlesize=titlesize, layout=layout)
    ...
    if isinstance(img, (list, tuple)):
        if layout is None:
            ...
            pass
        assert len(layout) == 2, \
            "Layout should be length-two tuple."

        fig, _ = _plt.subplots(figsize=[d * size//2 for d in reversed(layout)], sharex=True, sharey=True)
        for ix, p in enumerate(img, start=1):
            usekwargs=dict(**kwargs)
            leftColumn  = ((ix - 1) % layout[1] == 0)
            rightColumn = ((ix - 1) % layout[1] == (layout[1] - 1))
            if not leftColumn:
                usekwargs.pop('ylabel', None)
                pass
            if not rightColumn:
                usekwargs.pop('colorbar', None)
                pass
            image(p, ax=_plt.subplot(layout[0], layout[1], ix), **usekwargs)
            pass
        pass

    # If input is an array of images
    elif len(_np.squeeze(img).shape) > 2 and _np.squeeze(img).shape[2] not in [1, 3]:
