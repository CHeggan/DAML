...
class BayesianOptimiser:
    ...
    def __init__ (self, objective, dimensions):
       ...
       # Member variable(s)
        ...
        self.plotmesh = _np.meshgrid(*[_np.linspace(d.low, d.high, 200 + 1, endpoint=True) for d in self.dimensions])
        self.X        = _np.stack([a.flatten() for a in self.mesh]).T
        self.plotX    = _np.stack([a.flatten() for a in self.plotmesh]).T
